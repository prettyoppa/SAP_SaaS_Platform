...
가. 회원 가입·본인 확인, 로그인, 계정·프로필 관리, 탈퇴 및 유예 기간 처리
나. 개발 요청·인터뷰·제안서·FS·납품·문의/리뷰·오퍼·매칭 등 서비스 기능 제공
다. AI 기능 제공(외부 AI API 연동), AI 크레딧 잔액·충전·사용 내역 관리
라. 업무 알림·마케팅 알림(회원 동의 시), SMS·이메일 발송
마. 고객 문의·분쟁·부정 이용 방지(체험 플랜 중복 가입 방지 포함)
바. 서비스 개선, 로그·통계, 보안·장애 대응
사. 법령상 의무 이행
...
【선택】
· 회사명, 휴대전화번호(인증 시), 시간대(timezone) 설정
· 컨설턴트: 프로필 첨부 파일, 오퍼·문의 내용
· 알림 수신 동의: 업무 이메일/SMS, 마케팅 이메일/SMS (동의·철회 시각 기록)
· 계좌이체 충전: 입금자명, 입금 금액·일자 등 신청 시 입력 정보

【자동 수집】
· 쿠키·로컬 저장소: 언어(ko/en), 테마, 일부 UI 설정
· 서버 로그, 기기·브라우저 정보(보안·통계 목적)

비밀번호는 복호화 불가능한 방식으로 저장하며, 가능한 경우 첨부·업로드 파일은 클라우드 스토리지(R2 등)에 저장됩니다.
...
| 결제·충전 신청 | 전자상거래 등 관련 법령에 따른 보존 기간 |
| 접속 로그 | 보안 목적 최대 〔예: 1년〕 |
| Experience 체험 부정 재가입 방지 | 탈퇴·삭제 후에도 이메일·휴대폰의 **원문이 아닌** salted 일방향 해시만 〔예: 3년 또는 목적 달성 시까지〕 보관할 수 있음 |
...
회사는 원칙적으로 회원 개인정보를 외부에 제공하지 않습니다. 다만 다음의 경우는 예외입니다.

· 회원이 사전에 동의한 경우
· 법령에 따른 수사·조사 기관의 요청
· 통계·연구 목적의 비식별 정보

컨설턴트·요청 회원 간 문의 내용은 상대방에게 서비스 기능 범위 내에서 전달될 수 있으며, 회사 정책에 따라 발신자 표시가 제한될 수 있습니다.
...
국외 이전 시 이전 받는 자, 이전 국가, 이전 항목·목적·보유 기간을 별도 고지하거나 본 방침에 명시합니다. 이용자는 동의 거부 시 AI·알림 일부 기능 이용이 제한될 수 있습니다.
