/**
 * 요구사항·KB 본문 리치 에디터(인라인 이미지 붙여넣기) / 텍스트 전용 전환 — .req-rich-field 단위 초기화
 */
(function () {
  'use strict';
  ...
  function initReqRichRoot(root) {
    if (!root) return;

    var fmtInput = root.querySelector('.req-rich-fmt-input');
    var hiddenDesc = root.querySelector('.req-rich-hidden-body');
    var richEl = root.querySelector('.req-rich-surface');
    var plainEl = root.querySelector('.req-rich-plain');
    var toolbar = root.querySelector('.req-rich-toolbar');
    var alertEl = root.querySelector('.req-rich-alert');
    ...
    var modeRich = root.querySelector('.req-rich-mode-rich-input');
    var modePlain = root.querySelector('.req-rich-mode-plain-input');
    var formEl = root.closest('form') || document.getElementById(formId);
    var nonHtmlFmt = (root.getAttribute('data-rich-non-html-value') || 'plain').trim() || 'plain';
    ...
    function applyRich(rich) {
      if (fmtInput) fmtInput.value = rich ? 'html' : nonHtmlFmt;
      ...
      if (rich && plainEl && plainEl.value.trim() && richEl && !(richEl.innerText || '').trim()) {
        richEl.innerHTML = '<p>' + escapeHtml(plainEl.value).replace(/\n/g, '<br>') + '</p>';
      }
      if (!rich && richEl && plainEl && (richEl.innerText || '').trim()) {
    ...
    if (modeRich) {
      modeRich.addEventListener('change', function () {
        if (modeRich.checked) applyRich(true);
      });
    }
    if (modePlain) {
      modePlain.addEventListener('change', function () {
        if (modePlain.checked) applyRich(false);
      });
    }
