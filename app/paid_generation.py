...
def _fs_heartbeat(rfp_id: int, stop: threading.Event, n_holder: dict) -> None:
    """David 단계가 한 번에 오래 걸릴 때 주기 상태 메시지."""
    while True:
        ...
        append_delivery_job_log_line(
            rfp_id,
            "fs_job_log",
            f"David(FS)·Gemini 호출 진행 중… (heartbeat #{n_holder['count']}, 약 {_HEARTBEAT_SEC}초마다 표시)",
        )
...
def run_fs_generation_job(rfp_id: int) -> None:
    ...
    try:
        ...
        append_delivery_job_log_line(
            rfp_id,
            "fs_job_log",
            "David 에이전트(Gemini) FS 작성 호출 직전 · 수 분 걸릴 수 있음",
        )
        try:
            ...
            rfp.fs_error = None
            append_delivery_job_log_line(rfp_id, "fs_job_log", "David 단계 성공 · fs_text DB 저장 후 완료")
...
def run_delivered_code_job(rfp_id: int) -> None:
    ...
    try:
        ...
        append_delivery_job_log_line(
            rfp_id,
            "delivered_job_log",
            f"코드 생성 FS 원본 확인 완료 (길이 약 {len(fs_body.strip())}자)",
        )
        ...
        try:
            ...
            append_delivery_job_log_line(
                rfp_id,
                "delivered_job_log",
                "전체 에이전트 체인 성공 · delivered_code_text 저장",
            )
        except Exception as ex:
            rfp.delivered_code_status = "failed"
